...
void buscarRecursivo(int indice, char inicial) {
    // Caso base: si llegamos al final del arreglo, paramos
    if (indice >= total_contactos) {
        return;
    }

    // Comparamos la inicial del nombre del contacto actual
    if (contactos[indice].nombre[0] == inicial || 
        contactos[indice].nombre[0] == inicial - 32 || 
        contactos[indice].nombre[0] == inicial + 32) {
        
        printf("-> %s %s | Tel: %s | Apodo: %s\n", 
                contactos[indice].nombre, contactos[indice].apellido, 
                contactos[indice].telefono, contactos[indice].apodo);
    }

    // Llamada recursiva para el siguiente contacto
    buscarRecursivo(indice + 1, inicial);
}
